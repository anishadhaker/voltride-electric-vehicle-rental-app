...
function Payment() {
  ...
  const totalAmount = (Number(booking.rentalPrice) || 0) + (Number(booking.serviceFee) || 0) + (Number(booking.taxes) || 0);
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 pb-24 pt-32 lg:px-8">
      ...
      <div className="mt-8 rounded-3xl bg-white p-6 shadow-xl ring-1 ring-gray-100 sm:p-10">
        ...
        <div className="mt-8 space-y-3 text-sm"><div className="flex justify-between text-gray-600"><span>Rental amount</span><strong>Rs. {booking.rentalPrice}</strong></div><div className="flex justify-between text-gray-600"><span>Service fee</span><strong>Rs. {booking.serviceFee}</strong></div><div className="flex justify-between text-gray-600"><span>Taxes</span><strong>Rs. {booking.taxes}</strong></div><div className="flex justify-between border-t border-gray-200 pt-4 text-lg font-extrabold text-gray-950"><span>Total</span><span className="text-lime-700">Rs. {totalAmount}</span></div></div>
        ...
        {error && !isPickupPast && <p className="mt-5 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p>}
        <button type="button" disabled={processing || isPaid || isPickupPast} onClick={payNow} className="mt-8 w-full rounded-xl bg-gray-950 px-5 py-3.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{processing ? "Processing payment..." : isPaid ? "Payment complete" : isPickupPast ? "Booking expired" : `Pay now with ${paymentMethods.find((method) => method.id === selectedMethod)?.label}`}</button>
      </div>
    </main>
  );
}
